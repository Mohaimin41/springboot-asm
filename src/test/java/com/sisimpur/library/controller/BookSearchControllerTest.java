...
public class BookSearchControllerTest {
    ...
    void testSearchBooks_NoFilters_ReturnsAll() throws Exception {
        List<BookDTO> mockBooks = List.of(
                new BookDTO(1L, "Book A", "Fiction", 2022, "Author X"),
                new BookDTO(2L, "Book B", "Sci-Fi", 2020, "Author Y"));

        Mockito.when(bookSearchService.searchBooks(null, null, null, null)).thenReturn(mockBooks);
    ...
    void testSearchBooks_WithAuthorName() throws Exception {
        ...
        Mockito.when(bookSearchService.searchBooks(eq("X"), isNull(), isNull(), isNull()))
    ...
    void testSearchBooks_WithCategoryAndYear() throws Exception {
        List<BookDTO> mockBooks = List.of(
                new BookDTO(1L, "Book C", "Fantasy", 2021, "Author Z"));

        Mockito.when(bookSearchService.searchBooks(
                isNull(), isNull(), eq("Fantasy"), eq(2021)))
                .thenReturn(mockBooks);
    ...
    void testSearchBooks_WithAllFilters() throws Exception {
        List<BookDTO> mockBooks = List.of(
                new BookDTO(3L, "Mystery Novel", "Mystery", 2020, "Jane Doe"));

        Mockito.when(bookSearchService.searchBooks(
                eq("Jane Doe"), eq("Mystery"), eq("Mystery"), eq(2020)))
                .thenReturn(mockBooks);
